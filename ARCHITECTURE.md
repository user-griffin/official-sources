...
The request flow is: validate and migrate the config token → parse an IMDb or TMDB media ID → resolve the Watchmode title and network context → request normalized offers → validate content-level destinations → apply user filters → collapse exact duplicates → rank the detected home provider first → map to standard Stremio external streams. Episode responses are matched against the requested season/episode; when an exact destination is unavailable, a separate, explicitly labeled title-level result may be returned. Service roots and generic landing pages are rejected. Watchmode response objects are Zod-validated and do not cross the adapter boundary.
